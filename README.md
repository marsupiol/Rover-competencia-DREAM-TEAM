...
7. **Límite de Escala del Mapa Persistente (grilla densa):** resuelto con la ventana rodante (`rolling_window_enabled: true` en `persistent_map_params.yaml`). La grilla mantiene tamaño constante (400m × 400m @ 0.20m/px ≈ 4M celdas, ~16MB float32) sin importar la distancia recorrida; el decaimiento y la publicación operan sólo sobre esa ventana local. Para trayectos largos activar además `enable_road_routing:=true` (nivel 1 OSRM).
...
- [x] **Arquitectura de Tres Niveles Jerárquicos para Trayectos Largos (ej. 80km entre dos ciudades/universidades):** implementada en `er_planning`:
  - **Nivel 1 — `road_router_node`:** ruteo vial por grafo OSM vía OSRM; publica waypoints intermedios en `earth_rover/routing_waypoint`. Activar con `enable_road_routing:=true` en `mission1.launch.py` (default `false` para competencia corta).
  - **Nivel 2 — `persistent_map_node` + `global_planner_node`:** ventana rodante (`rolling_window_enabled: true`) de tamaño constante (400m × 400m) que re-centra alrededor del rover y descarta celdas atrás; D* Lite se reinicia automáticamente ante desplazamientos de origen.
  - **Nivel 3 — `bev_planner_node`:** planificador local reactivo BEV (4m, ya existente).
