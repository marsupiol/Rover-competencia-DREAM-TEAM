...
| **3. Mapeo Acelerador $\leftrightarrow$ Velocidad** | Velocidad lineal real en régimen permanente $v_{\text{ss}}$ (m/s) para escalones de acelerador $T \in [0.10, 1.00]$ en pasos de $0.05$, determinando la zona muerta exacta `motor_deadband_throttle` (estimada preliminarmente en $0.15$). | Corridas en línea recta de $10\text{ m}$ registrando posición geodésica RTK / odometría visual externa vs tiempo, con batería al 100%, 75% y 50%. | Construcción de una curva de calibración estática (LUT o función spline) que sustituya la equivalencia lineal $v = 1.111 \cdot T$, permitiendo control cinemático real en m/s y fijación empírica del piso de zona muerta. |
| **4. Sesgos Estáticos de IMU (COMPLETADA)** | Sesgo constante de giróscopo ($b_{gx}, b_{gy}, b_{gz}$) y acelerómetro ($b_{ax}, b_{ay}, b_{az}$) del chip MPU-6050. | **EJECUTADA (Brief 25)**: Captura continua en reposo absoluto de $125.09\text{ s}$ (1.235 muestras). Deriva en 120s $< 0.005^\circ/\text{s}$. Archivos generados y cargados: `config/gyro_bias.json` y `config/accel_bias.json`. Factor de escala MPU6050 calibrado a $0.5098$. | Supresión del drift de inclinación en el filtro complementario (duty cycle de compuerta al 100%) y compensación estricta de velocidad angular en `/imu/data`. |
